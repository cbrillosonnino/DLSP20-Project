...
from model_bb import Yo2o, Yo4o
...
def main():
    ...
    trainloader, valloader = get_loaders('labeled', batch_size = args.batch_size, 
            image_folder = '../data', annotation_file = '../data/annotation.csv')

    model = Yo4o(args.feature_size, args.num_bboxes, device, load_pretrained = True).to(device)
    loss_fxn = Loss(args.feature_size, args.num_bboxes, args.lambda_coord, args.lambda_noobj)
    
    params = list(list(model.lin1.parameters()) + list(model.lin2.parameters())+list(model.classifier.parameters()))
