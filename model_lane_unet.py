...
class Multi_UNet(nn.Module):
    def __init__(self, n_channels, n_classes, BEV=True, Bilinear=True, concat=False):
        ...
        self.proj = nn.Linear(256*6, 256, bias=False)
    ...
    def forward(self, x, M_matrices):
        ...
        data = torch.cat(data, dim=0) #[6, bs, 256, h, w]
        print(data.shape)
        #pooling layer from multi-view. 
        if self.concat:
            agg = self.proj(data.permute(1,3,4,2,0).reshape(len(x), 27, 38, 256*6))  #first reshpe to [bs,h,w,256,6] then squeeze to [bs,h,w,256*6]
            agg = agg.permute(0,3,1,2)
        else:
            agg = torch.max(data,dim=0)[0] #get the max values among 6 views, shape: [batch_size, 256, 16, 19]
        #interpolate up
        x = self.up1(agg)
        x = self.up2(x)
        #x = self.up3(x)
